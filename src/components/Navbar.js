...
const Navbar = () => {
    ...
    return (
        <div className=" bg-black flex justify-between items-center w-full fixed z-10 h-12">
            ...
            <div onClick={() => setsmallMenu(!smallMenu)} className="cursor-pointer pr-4 z-10 text-gray-500 sm:hidden">
            ...
            {smallMenu && (
                <ul className="flex flex-col justify-center items-center absolute top-0 left-0 w-full h-screen bg-gradient-to-b from-black to-gray-800 text-gray-500 duration-1000">
                    {links.map(({ id, link }) => (
                        <li key={id} className="px-4 cursor-pointer py-6 text-2xl duration-200 hover:scale-125">
                            <Link onClick={() => setsmallMenu(!smallMenu)} to={link} smooth duration={500}>{link}</Link>
                        </li>
                    ))}
                </ul>
            )}
